...
func TestCacheEnforceMaxSizeZeroRemovesAll(t *testing.T) {
	c := &cache{
		vaults: make(map[string]*VaultMetadata),
	}
	...
	// Ensure precondition
	if c.size() != 3 {
		t.Errorf("expected cache size 3 before enforcement but got %d", c.size())
	}

	// Enforce max size of 0 should remove all entries
	c.enforceMaxSize(0)

	// Verify all entries were removed
	if c.size() != 0 {
		t.Errorf("expected cache size 0 after enforceMaxSize(0) but got %d", c.size())
	}
}
