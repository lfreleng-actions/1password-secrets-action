...
type Config struct {
	...
	SensitiveByDefault bool // Force sensitive context for all logs
}
...
func isGitHubActions() bool {
	return os.Getenv("GITHUB_ACTIONS") == trueString ||
...
func NewWithConfig(config Config) (*Logger, error) {
	...
	if config.Debug {
		debugHandler := slog.NewTextHandler(secureOutput, &slog.HandlerOptions{
...
func scrubKnownSecrets(message string) string {
	...
	if strings.Contains(scrubbed, "ops_") {
		// Match 1Password service account tokens (860 chars total)
		// Real tokens: ops_ + 856 chars = 860 total
		tokenPattern := regexp.MustCompile(`(ops_[a-zA-Z0-9+/=_-]{856})`)
	...
	envPattern := regexp.MustCompile(`(?i)(\b[A-Za-z_]*(?:PASSWORD|SECRET|TOKEN|KEY)\s*=\s*["']?)([^\s"'<>]{8,})(["']?)`)
...
func (l *Logger) Info(msg string, args ...any) {
	if l.config.SensitiveByDefault {
...
func (l *Logger) LogError(err error, message string, context map[string]interface{}) {
	...
	// Use sensitive context to ensure scrubbing of string values in maps
	l.ErrorSensitive(message, logData)
}

// LogErrorForUser logs an error with user-friendly formatting
func (l *Logger) LogErrorForUser(err error, userMessage string) {
	// Log detailed error for system logs with sensitive context to scrub values
	l.ErrorSensitive("Operation failed", "error", err.Error(), "user_message", userMessage)
...
func (l *Logger) LogRecoveredPanic(recovered interface{}, context map[string]interface{}) {
	...
	l.ErrorSensitive("Panic recovered", logData)
	...
	if outputFile := os.Getenv("GITHUB_OUTPUT"); outputFile != "" {
		...
		defer func() {
			if closeErr := file.Close(); closeErr != nil {
				l.ErrorSensitive("Failed to close GITHUB_OUTPUT file", "error", closeErr)
			}
		}()
...
func (l *Logger) GitHubSummary(content string) error {
	// Check if summary output is suppressed (e.g., in CI)
	if os.Getenv("OP_SUPPRESS_SUMMARY") == trueString {
		l.Debug("GitHub summary output suppressed by OP_SUPPRESS_SUMMARY")
		return nil
	}
	...
	defer func() {
		if closeErr := file.Close(); closeErr != nil {
			l.ErrorSensitive("Failed to close GITHUB_STEP_SUMMARY file", "error", closeErr)
		}
	}()
...
func (l *Logger) GitHubSummarySuccess(title string, metrics map[string]interface{}) error {
	...
	// Always print the heading with the key emoji
	content.WriteString(fmt.Sprintf("🔑 **%s**\n\n", title))

	// Suppress the metrics list if all values are effectively zero
	allZero := true
	for _, v := range metrics {
		s := strings.TrimSpace(fmt.Sprint(v))
		if s != "0" && s != "0.0%" && s != "0.00%" && s != "0s" && s != "0ms" {
			allZero = false
			break
		}
	}

	if len(metrics) > 0 && !allZero {
...
func (l *Logger) GitHubSetEnv(name, value string) error {
	...
	if envFile := os.Getenv("GITHUB_ENV"); envFile != "" {
		...
		defer func() {
			if closeErr := file.Close(); closeErr != nil {
				l.ErrorSensitive("Failed to close GITHUB_ENV file", "error", closeErr)
			}
		}()
...
func IsSecretValue(value string) bool {
	// Only flag 1Password service account tokens with exact format (860 chars)
	return strings.HasPrefix(value, "ops_") && len(value) == 860
}
