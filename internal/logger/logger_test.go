...
func TestScrubKnownSecrets(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "real 1password service account token (860 chars) would be scrubbed",
			input:    "Token: " + strings.Repeat("ops_", 1) + strings.Repeat("x", 856), // Exactly 860 chars
			contains: "ops_***xxxx",
		},
		{
...
func TestGitHubActionsMethods(t *testing.T) {
	...
	config := DefaultConfig()
	config.StandardizeOutput = false
	logger, err := NewWithConfig(config)
	if err != nil {
		t.Fatalf("Failed to create logger: %v", err)
	}
...
func TestIsSecretValue(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want:  false, // Only 860-character tokens are considered secrets
		},
		{
			name:  "real 1password service account token (860 chars)",
			value: strings.Repeat("ops_", 1) + strings.Repeat("x", 856), // Exactly 860 chars
		...
		},
		{
			...
			want:  false, // Only 860-character ops_ tokens are considered secrets
		},
		{
			name:  "empty string",
			value: "",
