...
func DefaultAuditConfig() *Config {
	inCI := os.Getenv("GITHUB_ACTIONS") == "true" || os.Getenv("GITHUB_WORKSPACE") != ""
	// Disable file logging by default on GitHub Actions; allow opt-in via OP_AUDIT_LOG_TO_FILE=true
	logToFile := !inCI
	if os.Getenv("OP_AUDIT_LOG_TO_FILE") == "true" {
		logToFile = true
	}
...
func (at *Trail) flushBuffer() {
	...
	for _, event := range events {
		...
		if err != nil {
			at.logger.ErrorSensitive("Failed to marshal audit event", "error", err)
			continue
		}

		if _, err := at.file.Write(append(eventJSON, '\n')); err != nil {
			at.logger.ErrorSensitive("Failed to write audit event", "error", err)
		}
	}

	// Force sync to disk
	if err := at.file.Sync(); err != nil {
		at.logger.ErrorSensitive("Failed to sync audit file", "error", err)
	}
}
