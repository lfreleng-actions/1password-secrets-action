...
func TestIsLikelyItemID(t *testing.T) {
	tests := []struct {
		name string
		s    string
		want bool
		...
		{
			name: "lowercase alnum 26",
			s:    strings.Repeat("a", 13) + strings.Repeat("1", 13),
			want: true,
		},
		{
			name: "length 25",
		...
		{
			name: "length 27",
			s:    strings.Repeat("a", 27),
			want: false,
		},
		{
			name: "uppercase present",
			s:    strings.Repeat("a", 25) + "A",
			want: false,
		},
		{
			name: "non-alnum hyphen",
			s:    strings.Repeat("a", 25) + "-",
			want: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isLikelyItemID(tt.s)
			if got != tt.want {
				t.Errorf("isLikelyItemID(%q) = %v, want %v", tt.s, got, tt.want)
			}
		})
	}
}
