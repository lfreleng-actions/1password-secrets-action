...
func TestManagerVersion(t *testing.T) {
	...
	if manager.Version() != "2.31.1" {
		t.Errorf("Version() returned %s, want 2.31.1", manager.Version())
	}
}
...
func TestDownloadFile(t *testing.T) {
	...
	ctx := context.Background()
	if downloadErr := manager.downloadFile(ctx, server.URL, tempFile); downloadErr != nil {
		t.Errorf("downloadFile() failed: %v", downloadErr)
	}
