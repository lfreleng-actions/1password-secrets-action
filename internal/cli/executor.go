...
func (e *Executor) Execute(ctx context.Context, args []string, opts *ExecutionOptions) (*ExecutionResult, error) {
	...
	result, err := e.executeCommand(cmd, pipes, execParams.opts, execParams.ctx)

	// Ensure process is cleaned up on context cancellation
	if execParams.ctx.Err() != nil && cmd.Process != nil {
		_ = e.killProcess(cmd.Process)
	}
...
func (e *Executor) executeCommand(cmd *exec.Cmd, pipes *commandPipes, opts *ExecutionOptions, execCtx context.Context) (*ExecutionResult, error) {
	...
	// Capture output with timeout monitoring
	stdoutResult, stderrResult, captureErr := e.captureAllOutputWithTimeout(pipes, execCtx)
	if captureErr != nil {
...
func (e *Executor) captureAllOutput(pipes *commandPipes, ctx context.Context) (*security.SecureString, *security.SecureString, error) {
	...
	go func() {
		defer wg.Done()
		stdoutResult, stdoutErr = e.captureOutput(pipes.stdout, ctx)
	}()

	go func() {
		defer wg.Done()
		stderrResult, stderrErr = e.captureOutput(pipes.stderr, ctx)
	}()
...
// captureAllOutputWithTimeout captures output with aggressive timeout handling
func (e *Executor) captureAllOutputWithTimeout(pipes *commandPipes, ctx context.Context) (*security.SecureString, *security.SecureString, error) {
	type result struct {
		stdout *security.SecureString
		stderr *security.SecureString
		err    error
	...
	go func() {
		stdout, stderr, err := e.captureAllOutput(pipes, ctx)
	...
	case <-ctx.Done():
		// Context canceled - close pipes to unblock readers
		e.closePipes(pipes)
		// Wait a short time for goroutines to exit
		select {
		case res := <-resultChan:
			return res.stdout, res.stderr, ctx.Err()
		case <-time.After(1 * time.Second):
			return nil, nil, fmt.Errorf("output capture timeout: %w", ctx.Err())
		}
	}
}
...
// captureOutput securely captures output from a reader with context cancellation support.
func (e *Executor) captureOutput(reader io.Reader, ctx context.Context) (*security.SecureString, error) {
	var lines []string
	...
	go func() {
		for scanner.Scan() {
			...
			if len(lines) > 10000 {
				scanErr = fmt.Errorf("output too large: too many lines")
				scanDone <- true
				return
			}
	...
	case <-ctx.Done():
		// Context canceled - try to close reader if possible
...
func (e *Executor) killProcess(process *os.Process) error {
	if process == nil {
		return nil
	}
