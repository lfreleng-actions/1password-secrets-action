...
func TestValidateToken(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "valid 860-character token",
			token:     testdata.ValidDummyToken,
			expectErr: false,
		},
		{
			name:      "valid ops token",
			token:     "ops_" + strings.Repeat("A", 856), // 860 chars total
			expectErr: false,
		},
		{
			name:      "empty token",
		...
		},
		{
			...
			token:     "invalid_" + strings.Repeat("B", 852), // 860 chars with invalid prefix
			expectErr: true,
		...
		},
		{
			...
			token:     "dummy_" + strings.Repeat("$", 854), // 860 total with invalid chars
			expectErr: true,
			errReason: "command injection patterns", // This gets caught by security validation first
		},
		{
			name:      "token with spaces",
			token:     "dummy_" + strings.Repeat(" ", 854), // 860 total with spaces
			expectErr: true,
			errReason: "Token format is invalid",
		},
	}
...
func TestParseSingleRecord(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			record:        "api-key/connection string",
			expectErr:     false,
			expectedName:  "api-key",
			expectedField: "connection string",
			expectedVault: "",
		},
		{
			name:          "both secret and field with spaces",
			record:        "Database Config/connection string",
			expectErr:     false,
			expectedName:  "Database Config",
			expectedField: "connection string",
			expectedVault: "",
		...
		{
			name:          "vault override with spaces in secret and field",
			record:        "Test Vault:Test Credential/api key",
			expectErr:     false,
			expectedName:  "Test Credential",
			expectedField: "api key",
			expectedVault: "Test Vault",
		},
	}
