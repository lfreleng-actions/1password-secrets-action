...
func TestMigrateConfig(t *testing.T) {
	...
	if writeErr := os.WriteFile(configPath, data, 0600); writeErr != nil {
		t.Fatalf("Failed to write old config: %v", writeErr)
	}
...
func TestImportConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if writeErr := os.WriteFile(inputPath, data, 0600); writeErr != nil {
...
func TestCleanupOldConfigs(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if infoErr != nil {
			t.Errorf("Failed to get file info for %s: %v", entry.Name(), infoErr)
			continue
		}
