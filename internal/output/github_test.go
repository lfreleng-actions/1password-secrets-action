...
func TestWriteToFile_Multiline(t *testing.T) {
	...
	// Check that it contains the multiline output with some delimiter
	assert.Contains(t, contentStr, "multiline_output<<EOF")
	assert.Contains(t, lines, "line1")
	assert.Contains(t, lines, "line2")
	assert.Contains(t, lines, "line3")

	// Find the delimiter used and verify it's properly closed
	var delimiter string
	for _, line := range lines {
		if strings.HasPrefix(line, "multiline_output<<") {
			delimiter = strings.TrimPrefix(line, "multiline_output<<")
			break
		}
	}
	assert.NotEmpty(t, delimiter, "Should have found a delimiter")
	assert.Contains(t, lines, delimiter)
}
...
func TestGenerateDelimiter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Should start with EOF
			assert.True(t, strings.HasPrefix(delimiter, "EOF"), "Delimiter should start with EOF")

			// Should not conflict with the input value
			assert.False(t, strings.Contains(tt.value, delimiter), "Delimiter should not appear in the input value")

			// Should be either "EOF" or "EOF_" followed by some suffix
			if delimiter != "EOF" {
				assert.True(t, strings.HasPrefix(delimiter, "EOF_"), "Multi-part delimiter should start with EOF_")
				assert.True(t, len(delimiter) > 4, "Multi-part delimiter should have a suffix")
			}
		})
	}
}
...
func TestDryRun(t *testing.T) {
	...
	if config.OutputFile != "" {
		content, readErr := os.ReadFile(config.OutputFile)
		if readErr == nil {
