...
func TestUnsupportedCLIVersionExitMessage(t *testing.T) {
	// Set up minimal GitHub Actions environment
	setupMinimalGitHubActionsEnv(t)
	defer cleanupGitHubActionsEnv(t)
	...
	// Force an unsupported CLI version
	_ = os.Setenv("INPUT_CLI_VERSION", "9.9.9")

	// Ensure no external override points to a DB containing this version
	_ = os.Unsetenv("OP_SECRETS_ACTION_VERSIONS_FILE")

	defer func() {
		_ = os.Unsetenv(EnvInputToken)
		_ = os.Unsetenv(EnvInputVault)
		_ = os.Unsetenv(EnvInputRecord)
		_ = os.Unsetenv("INPUT_CLI_VERSION")
	}()

	// Run the action; expect a clear unsupported-version error surfaced at command layer
	err := runAction(rootCmd, []string{})
	assert.Error(t, err)
	assert.Contains(t, strings.ToLower(err.Error()), "unsupported 1password cli version")
}
