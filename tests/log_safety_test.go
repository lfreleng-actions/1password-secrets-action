...
func TestLogSafety_EndToEnd_NoSecretInDebugLogs(t *testing.T) {
	...
	if _, processErr := outMgr.ProcessSecrets(results); processErr != nil {
		t.Fatalf("ProcessSecrets returned error: %v", processErr)
	}
	...
	if !strings.Contains(strings.ToLower(logContent), "retrieving secret") {
