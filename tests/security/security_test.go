...
func (s *SecurityTestSuite) TestInputValidationAttacks() {
	maliciousInputs := []struct {
	...
	}{
		{
			...
			token:       testdata.ShortDummyToken + "'; DROP TABLE secrets; --",
			vault:       "test-vault",
			record:      "test/field",
			shouldFail:  true,
		...
		},
		{
			...
			token:       testdata.ShortDummyToken + "\x00",
			vault:       "test-vault",
			record:      "test/field",
			shouldFail:  true,
...
func (s *SecurityTestSuite) TestAccessControlSecurity() {
	s.Run("token_validation", func() {
		invalidTokens := []string{
			...
			testdata.ShortDummyToken, // Wrong length
		}
...
func (s *SecurityTestSuite) TestResourceExhaustionAttacks() {
	s.Run("memory_exhaustion_protection", func() {
		...
		cfg := &config.Config{
			ServiceAccountToken: testdata.ValidDummyToken,
			Vault:               "test-vault",
			Record:              largeRecord,
			ReturnType:          "output",
			Debug:               false,
...
func (s *SecurityTestSuite) TestInformationDisclosureAttacks() {
	s.Run("error_message_sanitization", func() {
		...
		cfg := &config.Config{
			ServiceAccountToken: testdata.ShortDummyToken,
	...
	s.Run("stack_trace_sanitization", func() {
		...
		defer func() {
			if r := recover(); r != nil {
				...
				assert.NotContains(s.T(), stackTrace, "dummy_",
