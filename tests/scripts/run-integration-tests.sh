...
ENVIRONMENT VARIABLES:
    OP_SERVICE_ACCOUNT_TOKEN    1Password service account token (will prompt if not set)
...
check_prerequisites() {
    ...
    if [[ -z "${OP_SERVICE_ACCOUNT_TOKEN:-}" ]]; then
        ...
        echo
        echo "Please provide a 1Password service account token for integration tests."
        echo "The token should be 860 characters starting with 'ops_'."
        echo
        read -r -p "Enter 1Password service account token: " -s token_input
        echo
        ...
        if [[ -n "$token_input" ]]; then
            ...
            log_error "No token provided - cannot run integration tests"
            exit 1
        fi
    else
    ...
    if [[ ! "$OP_SERVICE_ACCOUNT_TOKEN" =~ ^(ops_[a-zA-Z0-9+/=_-]{856}|dummy_[a-zA-Z0-9+/=_-]{854})$ ]]; then
        log_warning "Service account token format may be invalid"
        log_warning "Expected format: ops_<856 base64-encoded JWT characters> (860 total) or dummy_<854 chars> for testing"
    fi
...
setup_test_environment() {
    ...
    export OP_VAULT="${OP_VAULT:-Test Vault}"
    export OP_TEST_CREDENTIAL_1="${OP_TEST_CREDENTIAL_1:-Testing}"
    export OP_TEST_CREDENTIAL_2="${OP_TEST_CREDENTIAL_2:-Testing}"
