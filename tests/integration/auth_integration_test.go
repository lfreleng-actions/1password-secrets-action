...
func TestIntegrationVaultResolution(t *testing.T) {
	token := os.Getenv("OP_SERVICE_ACCOUNT_TOKEN")
	vaultName := os.Getenv("OP_VAULT")
	if token == "" || vaultName == "" {
		t.Skip("OP_SERVICE_ACCOUNT_TOKEN or OP_VAULT not set, skipping integration test")
	}
...
func TestIntegrationAccessValidation(t *testing.T) {
	token := os.Getenv("OP_SERVICE_ACCOUNT_TOKEN")
	vaultName := os.Getenv("OP_VAULT")
	itemName := os.Getenv("OP_TEST_CREDENTIAL_1")
	if token == "" || vaultName == "" || itemName == "" {
		t.Skip("OP_SERVICE_ACCOUNT_TOKEN, OP_VAULT, or OP_TEST_CREDENTIAL_1 not set, skipping integration test")
	}
