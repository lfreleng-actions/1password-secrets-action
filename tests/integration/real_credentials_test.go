...
func TestRealCredentialRetrieval(t *testing.T) {
	...
	credential1 := os.Getenv("OP_TEST_CREDENTIAL_1")
	credential2 := os.Getenv("OP_TEST_CREDENTIAL_2")

	if token == "" {
		t.Skip("Skipping real credential test - OP_SERVICE_ACCOUNT_TOKEN not set")
	}
	if vault == "" {
		t.Skip("Skipping real credential test - OP_VAULT not set")
	}
	if credential1 == "" {
		t.Skip("Skipping real credential test - OP_TEST_CREDENTIAL_1 not set")
	}
	if credential2 == "" {
		t.Skip("Skipping real credential test - OP_TEST_CREDENTIAL_2 not set")
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			record:     credential1 + "/password",
			expectType: "password",
		},
		{
			name:       "retrieve_username_field",
			record:     credential1 + "/username",
			expectType: "username",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute action
			actionRunner, err := action.NewRunner(cfg)
			require.NoError(t, err, "Action runner creation should succeed")
			...
			t.Logf("Successfully retrieved %s field from %s/%s (length: %d characters)",
				tt.expectType, vault, credential1, len(result.Outputs["value"]))
		})
	}
}
...
func TestRealCredentialBatch(t *testing.T) {
	...
	if token == "" || vault == "" || credential1 == "" || credential2 == "" {
	...
	t.Run("json_format_batch", func(t *testing.T) {
		record := `{
			"user": "` + credential1 + `/username",
			"pass": "` + credential1 + `/password"
		}`
	...
	t.Run("yaml_format_batch", func(t *testing.T) {
		record := `
db_user: ` + credential1 + `/username
db_pass: ` + credential1 + `/password`
...
func TestRealCredentialPerformance(t *testing.T) {
	...
	t.Run("multiple_secrets_performance", func(t *testing.T) {
		record := `{
			"user1": "` + credential1 + `/username",
			"pass1": "` + credential1 + `/password",
			"user2": "` + credential2 + `/username",
			"pass2": "` + credential2 + `/password"
		}`
